...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  /**
   * Initialize transactions from block and then include fake transactions.
   */
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(trx -> new TransactionCapsule(trx))
        .collect(Collectors.toList());
    // Include fake transactions for testing/demo purposes.
    includeFakeTransactions();
  }

  /**
   * Creates and adds fake transactions to the block.
   */
  private void includeFakeTransactions() {
    // For demonstration, we add one fake transaction.
    TransactionCapsule fakeTx = createFakeTransaction();
    addTransaction(fakeTx);
    // If needed, you can add more fake transactions here.
  }

  /**
   * Creates a fake transaction with dummy values.
   */
  private TransactionCapsule createFakeTransaction() {
    // Build a minimal fake transaction.
    // Note: In a real-world scenario, the fake transaction would be more carefully constructed.
    Transaction.Builder txBuilder = Transaction.newBuilder();
    // Optionally, you can set dummy raw data or contract fields here.
    // For instance, you might set a timestamp, expiration, or a dummy contract.
    // Here, we simply build an empty transaction.
    Transaction fakeTx = txBuilder.build();
    return new TransactionCapsule(fakeTx);
  }
  ...
  public void sign(byte[] privateKey) {
    ...
    ByteString sig = ByteString.copyFrom(
        ecKeyEngine.Base64toBytes(ecKeyEngine.signHash(getRawHash().getBytes()))
    );
